...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return <main className="mx-auto my-12 max-w-md space-y-5 rounded-xl border bg-card p-7 shadow-sm">
    ...
    {state === "login" && <form className="space-y-3" onSubmit={event => {event.preventDefault(); perform(async () => { await login(email,password); await check(); });}}>
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" disabled={busy || !email || password.length < 12} onClick={()=>perform(async()=>{await signup(email,password);setMessage("Check your email to confirm your account, then sign in. Use at least 12 characters for your password.");})}>Create owner account</Button><Button type="button" variant="outline" disabled={busy || !email} onClick={()=>perform(async()=>{await requestPasswordRecovery(email);setMessage("Check your email for the password-reset link.");})}>Forgot password</Button></div>
      <p className="text-xs text-muted-foreground">For first-time setup, use your approved owner email and a password of at least 12 characters.</p>
    </form>}
    {state === "password" && <form className="space-y-3" onSubmit={e=>{e.preventDefault();perform(async()=>{if(invite) await acceptInvite(invite,password);else await updateUser({password});await check();});}}><label>New password<input type="password" autoComplete="new-password" className="w-full rounded border p-2" minLength={12} value={password} onChange={e=>setPassword(e.target.value)} required /></label><Button disabled={busy}>Save password</Button></form>}
