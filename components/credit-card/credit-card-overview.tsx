...
export function CreditCardOverview() {
  ...
  const authorizedCards = cards.data?.cards.filter(c => c.transactionsAllowed) ?? [];
  const choices = authorizedCards.length ? authorizedCards : cards.data?.cards ?? [];
  const card = choices.find(c => c.accountId === selected) ?? choices[0];
  return <div className="space-y-6">
    ...
    {choices.length > 1 && <label className="block">Card<select className="ml-3 rounded border p-2" value={card?.accountId ?? ""} onChange={e => setSelected(e.target.value)}>{choices.map(c => <option key={c.accountId} value={c.accountId}>{c.name} {c.mask ? `ending ${c.mask}` : ""}</option>)}</select></label>}
...
function CardAnalysis({ card }: { card: LinkedCard }) {
  ...
  const canCompare = Boolean(earliest && earliest <= `${monthBefore(month)}-01` && startDate <= `${monthBefore(month)}-01`);
