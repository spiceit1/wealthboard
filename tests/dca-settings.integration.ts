...
async function main(){
 ...
 await saveDcaSettings(id,2,{...defaultDcaSettings,name:"Revised",reentryMode:"dip_rebound",reentryDip:4,reentryRebound:1},"draft");
 const state=await readDcaSettings(id);
 assert.equal(state.latest?.version,3);assert.equal(state.latest?.settings.reentryMode,"dip_rebound");assert.equal(state.latest?.settings.reentryDip,4);assert.equal(state.queued?.settings.reentryMode,"after_cooldown");assert.equal(state.queued?.version,2);assert.equal(state.queued?.settings.name,"Zcash DCA");assert.equal(state.activeVersion,null);assert.equal(state.liveTradingEnabled,false);
